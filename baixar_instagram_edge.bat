...
set "HEADLESS_ARG="
if exist ".\downloads_instagram\.playwright-instagram-session\msedge\Default" set "HEADLESS_ARG=--headless"

echo.
echo Baixando tudo pelo Edge...
if "%HEADLESS_ARG%"=="" echo Uma janela real do Edge vai abrir. Depois do login, deixe o perfil aberto que o bot recarrega e segue sozinho.
if not "%HEADLESS_ARG%"=="" echo Sessao encontrada. Rodando em segundo plano sem abrir janela.
".venv\Scripts\python.exe" ".\baixar_instagram.py" "%PERFIL%" --midia tudo --browser edge %HEADLESS_ARG%
if errorlevel 1 goto :erro
